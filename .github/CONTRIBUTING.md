...
Thanks for using `astronomy-mcp-server`. Bugs, feature requests, and documentation gaps all belong in an issue — that's where they get read and picked up.

- [Report a bug](https://github.com/cyanheads/astronomy-mcp-server/issues/new?template=bug_report.yml)
- [Request a feature](https://github.com/cyanheads/astronomy-mcp-server/issues/new?template=feature_request.yml)

Both forms are structured, and filling in the fields is what makes an issue actionable.

## Server bug or framework bug?

`astronomy-mcp-server` is built on [@cyanheads/mcp-ts-core](https://github.com/cyanheads/mcp-ts-core), which handles transports, auth, config, logging, and telemetry. Sorting out which layer broke saves everyone a round-trip:
