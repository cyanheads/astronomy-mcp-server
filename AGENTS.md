...
`stateless`, declared in `src/index.ts` via `createApp({ sessionMode: 'stateless' })`, set in the `Dockerfile`, and set actively in `.env.example`. The `src/` declaration is the durable form — the posture travels with the source rather than with whatever a deployment happens to export — and `MCP_SESSION_MODE` still wins when it carries a meaningful value, which is how an operator overrides it. The mode a stateless server gives up is the 2025-era multi-round-trip shim, and no handler here calls `ctx.requestInput` — every tool is a pure function of its arguments — so there is nothing to give up. That absent call site is also why `require: 'stateful'` is not declared: there is no capability worth failing startup over. stdio and 2026-07-28 clients behave identically under either mode, so a green field test says nothing about this choice; the absent call site is the reason.

No service allocates a watcher, socket, or ref'd timer — the satellite TLE cache is a plain `Map` whose entries expire on read — so `createApp({ teardown })` is deliberately not declared. Add it the moment a service grows a handle that outlives a request.
...
Most of this server's declared reasons are raised below the handler — `EphemerisService.resolveTime()` / `resolveTimezone()` / `lookupStar()`, `resolveBody()`, and the two extension services — and carry their hint inline because those call sites take no `ctx`. Those entries are marked `thrownBy: 'service'`; the handler-local preconditions (`find_events`'s `incomplete_observer` and `body_required`, `invalid_target`, the `get_ephemeris` argument gates, `unknown_body`) are the ones that throw through `ctx.fail`. A contract entry whose hint is written at a service throw site must repeat that string verbatim, since nothing links the two — `tests/tools/error-contract-coverage.test.ts` holds both halves of that line.
...
Skills are modular instructions in `framework-skills/` at the project root. Read them directly when a task matches — e.g., `framework-skills/add-tool/SKILL.md` when adding a tool. `bun run list-skills` prints the full registry. The directory is deliberately not `skills/`: Claude Code and Codex auto-load a plugin's root `skills/`, so a server that ships `.claude-plugin/` or `.codex-plugin/` — this one ships both — would hand these development skills to every agent that installs it. Keep `skills/` free for skills meant for those agents.
