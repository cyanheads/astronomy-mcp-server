...
export const stargazingPlanPrompt = prompt('astronomy_stargazing_plan', {
  ...
  generate: (args) => {
    ...
    const text = [
      ...
      '',
      `1. Resolve the location to latitude/longitude with a geocoding tool (e.g. openstreetmap_search_places), and its IANA timezone with a timezone lookup (e.g. ref_timezone_lookup, given the place's city or region). This astronomy server does not geocode. Pass that zone as the \`timezone\` parameter on every astronomy call below: without it the tools answer in UTC only, and you cannot tell which local night a returned instant belongs to.`,
